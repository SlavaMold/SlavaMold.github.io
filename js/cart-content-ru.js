...
$.getJSON('goods-ru.json', function (data) {
    var goods = data;
    console.log(goods);
    checkCart();
    checkQuantity();
    ...
    function showCart() {
        if ($.isEmptyObject(cart)) {
            ...
            out += '<h2 class="header-block__title"> Корзина пуста </h2>';
            comanda();
        ...
        function comanda() {
            if (!$.isEmptyObject(cart)) { 
                ...
                var out2 = '';
                out2 += '<form action="" method="" class="forms">';
                out2 += '<legend><h2 class="header-block__title">';
                out2 += 'Оформить заказ';
                out2 += '</h2></legend>';

                out2 += '<div class="form-group">';
                out2 += '<label for="">Имя, Фамилия</label>';
                out2 += '<input type="text" class="form-control" id="" name="user_name" placeholder="Например, Александр">';
                out2 += '</div>';

                out2 += '<div class="form-group">';
                out2 += '<label for="">Номер телефона</label>';
                out2 += '<input type="text" class="form-control" id="" name="user_phone" placeholder="+373 xxxxxx">';
                out2 += '</div>';

                out2 += '<div class="form-group">';
                out2 += '<label for="">E-mail</label>';
                out2 += '<input type="text" class="form-control" id="" name="user_email" placeholder="mail@mail.com">';
                out2 += '</div>';
                out2 += '<div class="form-group">';
                out2 += '<button type="submit" class="main_block__button_red2">Отправить</button>';
                out2 += '</div>';
                out2 += '</form>';
